...
class CreditPlan(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)  # Free, Basic, Premium, Enterprise
...
class UserCredit(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    plan_id = db.Column(db.Integer, db.ForeignKey('credit_plan.id'), nullable=False)
    ...
    plan = db.relationship('CreditPlan')
...
class UsageLog(db.Model):
    ...
    industry = db.Column(db.String(50))  # healthcare, finance, retail, etc.
...
class IndustryModel(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    industry_name = db.Column(db.String(100), nullable=False)  # healthcare, finance, retail, etc.
    model_type = db.Column(db.String(50), nullable=False)  # churn, lead_score, etc.
    model_version = db.Column(db.String(20), default='v1.0')
    model_path = db.Column(db.String(500))  # Path to model file
    is_active = db.Column(db.Boolean, default=True)
    ...
    feature_weights = db.Column(db.Text)    # JSON string of feature importance
    preprocessing_config = db.Column(db.Text)  # JSON string of preprocessing steps
...
class UserCredit(db.Model):
    ...
    # Relationships
    user = db.relationship('User', backref='user_credits')
    plan = db.relationship('CreditPlan', backref='user_credits')
